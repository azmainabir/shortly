        ...
        {shortUrl && (
          <div className="mt-3 space-y-2">
            <div className="p-4 bg-slate-800 rounded-xl flex items-center justify-between gap-4">
              <a href={shortUrl} target="_blank" className="text-violet-400 text-lg truncate">
                {shortUrl}
              </a>
              <button onClick={handleCopy} className="px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg transition-all">
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
            {showQR && (
